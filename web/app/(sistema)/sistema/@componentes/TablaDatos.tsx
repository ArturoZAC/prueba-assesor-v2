...
export function TablaDatos<TData extends RowData>({
  ...
  const table = useReactTable<TData>({
    ...
    state: {
      ...
      globalFilter,
      rowSelection,
      pagination: { pageIndex: 0, pageSize: safeLimit },
    },
  ...
  return (
    <div className="overflow-x-auto">
        ...
        <tbody>
          {table.getRowModel().rows.map((row) => (
            <tr
              ...
              onClick={() => {
                console.log("Fila seleccionada: ", row.original);
              ...
              {row.getVisibleCells().map((cell, index) => {
                ...
                if (index === 1) {
                  return (
                    ...
                    >
                      {/*@ts-ignore */}
                      {row.original.resaltarFila && row.original?.resaltarFila.active != undefined ? row.original?.resaltarFila.active && (
      ...
      {isMenuVisible && (
        <div
          className="overflow-hidden border rounded shadow text-black-main bg-white-main"
          style={{
            position: "absolute",
            left: menuPosition.x,
            top: menuPosition.y,
