...
import { Errors } from '@/components/Errors';
import { AgregarCuadreLeasingSchema } from '../schemas/AgregarCuadreLeasing';

export default function AgregarCuadreLeasing() {

  const { selectedRow, closeModal, setDatosOperaciones, datosOperaciones } = useAuth();
  ...
  useEffect(() => {
    if (rowExcelSelected) {
      ...
      setFieldValue("fecha", String(rowExcelSelected["Fecha"] || '')); 
      ...
      setFieldValue("diferencia", Number(selectedRow.monto_total) - Math.abs(Number(rowExcelSelected['Monto'])));
    }
  }, [rowExcelSelected, setFieldValue]);

  useEffect(() => {
    setFieldValue("montoFinal", values.pagado)
    setFieldValue("diferencia", Math.abs(Number(selectedRow.monto_total)) - Math.abs(Number(values.pagado)))
    setFieldValue("pagadoDet", Math.abs(Number(selectedRow.monto_total - values.pagado)))
    setFieldValue("montoFinalDet", Math.abs(Number(selectedRow.monto_total - Math.abs((Number(values.pagado))))))
            ...
            <div className="grid grid-cols-1 mb-5 gap-x-4 gap-y-6 md:grid-cols-3">
              ...
              <div className="w-full">
                <InputForm
                  ...
                  type="number"
                  disabled
                  value={Number(values.pagadoDet).toFixed(2)}
                />
              ...
              <div className="w-full">
                <InputForm
                  ...
                  placeholder="Tipo de Cambio (Detracción)"
                  type="number"
                  value={Number(values.montoFinalDet).toFixed(2)}
                  disabled
                />
              ...
              <div className="w-full">
                <InputForm
                  ...
                  type="number"
                  value={Number(values.referenciaDet).toFixed(2)}
                  onChange={handleChange}
                  onBlur={handleBlur}
                  disabled
                />
