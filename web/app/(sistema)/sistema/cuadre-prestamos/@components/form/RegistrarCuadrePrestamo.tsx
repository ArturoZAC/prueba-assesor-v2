...
export default function RegistrarCuadrePrestamo() {
  ...
  const agregarCuadrePrestamo = async (): Promise<void> => {
    try {
      ...
      const { status, data } = await axios.post(
        `${config.apiUrl}/cuadreprestamos/salida/${selectedRow?.id}`,
        {
          ...values,
        },
        {
  ...
  useEffect(() => {
    if (rowExcelSelected) {
      ...
      setFieldValue("diferencia", Number(Number(selectedRow.capital_dolares ?? 0) + Number(selectedRow.capital_soles ?? 0) - Math.abs(Number(rowExcelSelected['Monto'] ?? 0))) ?? 0);
    }
            ...
            <div className="grid grid-cols-1 mb-5 gap-x-4 gap-y-6 md:grid-cols-3">
              ...
              <div className="w-full">
                <InputForm
                  ...
                  step={0.01}
                  disabled
                  value={selectedRow?.capital_soles}
                />
              </div>
              <div className="w-full">
                <InputForm
                  ...
                  placeholder="Cpaital Dolares"
                  type="number"
                  step={0.01}
                  disabled
                  value={selectedRow?.capital_dolares}
                />
              </div>
              <div className="w-full">
                <InputForm
