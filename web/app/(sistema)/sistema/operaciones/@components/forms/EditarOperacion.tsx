...
export const EditarOperacion = () => {
  ...
  const obtenerUltimaOperacion = async () => {
    try {
      const request = await axios.get(
        `${config.apiUrl}/operaciones/ultimaOperacion/${selectedRow.numero}`
      );
      console.log(request.data.ultimaOperacion)
      setUltimaOperacion(request.data.ultimaOperacion);
    } catch (error) {
      console.log(error);
      if (error instanceof AxiosError) {
        toast.error(error.response?.data.error ?? 'Ha ocurrido un error, contacte con sistemas');
      }
  ...
  const agregarOperacion = async (): Promise<void> => {
    ...
    try {
      const { status, data } = await axios.post(
        `${config.apiUrl}/operaciones/editar/${selectedRow.id}`,
        {
          ...values,
          ...
          montoUSD: Number(values.montoUSD),
          movimiento_compraUSD: parseFloat(values.movimiento_compraUSD),
          movimiento_ventaUSD: parseFloat(values.movimiento_ventaUSD)
        },
        {
  ...
  const {
    ...
    initialValues: {
      ...
      dolares: selectedRow?.dolares,
      t: selectedRow?.t,

      cliente: selectedRow?.cliente,
      documento: selectedRow?.documento,

      compra: selectedRow?.tipoCambio_compra,
      venta: selectedRow?.tipoCambio_venta,
      spread: selectedRow?.tipoCambio_spread,
      promedio: selectedRow?.tipoCambio_promedio,
    ...
    validationSchema: registroOperacionSchema,
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="w-full ">
        <div className="grid grid-cols-1 mb-5 gap-x-4 gap-y-6 md:grid-cols-3">
          <div className="w-full">
            <InputForm
              ...
              placeholder="Fecha"
              type="date"
              value={values.fecha ? values.fecha.toString().split("T")[0] : ""}
              className=""
          ...
          <div className="w-full">
            <BusquedaCliente setUsuario={setUsuario} usuario={usuario} usuarioBuscado={selectedRow.usuario} />
          </div>
          <div className="w-full">
            <InputForm
              label="Documento"
        ...
        <div className="flex flex-col items-center w-full gap-4 mt-4 lg:flex-row">
          ...
          <div className="w-full lg:w-1/2">
            <ButtonSubmit loading={loading} text="Editar operación" />
          </div>
        </div>
      </div>
    </form>
