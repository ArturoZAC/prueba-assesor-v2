...
const SubirDatosFacturacion: React.FC<SubirDatosExcelProps> = ({
  fechaKey,
  headers,
  setData,
  textAlert,
  ...
  const { datosFacturacion, selectedRow } = useAuth();
  const [dataToDisplay, setDataToDisplay] =
    useState<ExcelData[]>(datosFacturacion);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      ...
      // obtenemos un array de rows incluyendo encabezados
      const raw: any[][] = XLSX.utils.sheet_to_json(ws, { header: 1 });
      const rows = raw.slice(1); // descartamos la fila de cabeceras

      // formateamos cada celda usando formatCellValue
      const formatted: ExcelData[] = rows.map((row) => {
        const obj: ExcelData = {};
        headers.forEach((header) => {
          const key = header.key; // Usamos key para el objeto
          const value = row[header.id - 1]; // Usamos id como índice de columna
          if (key === fechaKey) {
  ...
  async function eliminarDato() {
    try {
      const res = await axios.post(`${config.apiUrl}/${urlDelete}/${selectedRow.cuadreFacturacionId}`)
...
export default SubirDatosFacturacion;
