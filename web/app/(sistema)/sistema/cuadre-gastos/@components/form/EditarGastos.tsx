...
export const EditarGastos = () => {
  ...
  const [isRecopilado, setIsRecopilado] = useState<boolean>(selectedRow.isRecopilado)

  console.log("Row edit: ", selectedRow);
  ...
  const editarGasto = async (): Promise<void> => {
    try {
      setLoading(true);
      const { status } = await axios.put(
        `${config.apiUrl}/gastos/editar/${selectedRow.id}`,
        {
          ...values,
          isRecopilado
        },
  ...
  const {
    ...
    initialValues: {
      fecha: formatearFecha(selectedRow?.fecha),
      descripcion: selectedRow?.descripcion,
      monto: selectedRow?.monto,
      referencia: selectedRow?.referencia,
      tipoMoneda: selectedRow?.tipoMoneda,
      clase: selectedRow?.clase,
      concepto: selectedRow?.concepto,
      tipoGasto: selectedRow?.tipoGasto,
    },
    validationSchema: agregarGastoSchema,
    onSubmit: editarGasto,
  });

  console.log("Row excel: ", rowExcelSelected);

  useEffect(() => {
    if (
      rowExcelSelected &&
      rowExcelSelected["Monto"] !== "" &&
      rowExcelSelected["Descripción operación"] !== ""
    ) {
      setFieldValue(
        "fecha",
      ...
      setFieldValue("concepto", rowExcelSelected['Concepto'] ?? "");
    }
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="w-full ">
          ...
          <div className="w-full lg:w-1/2">
            <div className="w-full max-h-[410px] overflow-y-auto">
              <SubirDatosExcel
                ...
                fechaKey="fecha"
                nombreDelete="Eliminar Gasto"
                urlDelete="gastos/eliminar"
              />
            </div>
          </div>
        </div>
        <div className="flex flex-col items-center w-full gap-4 mt-4 lg:flex-row">
          ...
          <div className="w-full lg:w-1/2">
            <ButtonSubmit loading={loading} text="Editar gasto" />
          </div>
        </div>
      </div>
    </form>
