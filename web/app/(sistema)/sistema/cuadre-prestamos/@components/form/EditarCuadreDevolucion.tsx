...
export default function EditarCuadreDevolucion({ rowCuadre }: { rowCuadre: DevolucionesPrestamo }) {
  ...
  const [rowExcelSelected, setRowExcelSelected] = useState<any>({
    'Fecha': "",
    'Descripción operación': rowCuadre.deposito,
    'Monto': rowCuadre.pagado,
    'tc': rowCuadre.tc,
    'Referencia2': '',
  });
  ...
  const agregarCuadrePrestamo = async (): Promise<void> => {
    try {
      ...
      const { status, data } = await axios.post(
        `${config.apiUrl}/cuadreprestamos/devolucion/editar/${rowCuadre?.id}`,
        {
          ...values,
        },
        {
  ...
  const {
    ...
    initialValues: {
      fecha: formatoFecha(String(rowCuadre.fecha)),
      deposito: rowCuadre.deposito,
      pagado: rowCuadre.pagado,
      tc: rowCuadre.tc,
      montoFinal: rowCuadre.montoFinal,
      referencia: rowCuadre.referencia,
  ...
  useEffect(() => {
    if (rowExcelSelected) {
      ...
      setFieldValue("pagado", Math.abs(Number(rowExcelSelected['Monto'] ?? '')));
      setFieldValue("tc", rowExcelSelected['tc'] ?? '');
      setFieldValue("montoFinal", Math.abs(Number(rowExcelSelected['Monto'] ?? '')) ?? '');
      setFieldValue("referencia", rowExcelSelected['Referencia2'] ?? '');
      setFieldValue("monto", Math.abs(Number(rowExcelSelected['Monto'] ?? '')));
      setFieldValue("diferencia", Number(Number(selectedRow?.montoTotal || 0) - Math.abs(Number(rowExcelSelected['Monto'] ?? ''))) ?? 0);
    }
      ...
      <form onSubmit={handleSubmit} className="w-full">
        ...
        <div className="flex flex-col items-center w-full gap-4 mt-4 lg:flex-row">
          ...
          <div className="w-full lg:w-1/2">
            <ButtonSubmit
              loading={loading}
              text="Agregar Cuadre Devolución"
            />
          </div>
        </div>
      </form>
