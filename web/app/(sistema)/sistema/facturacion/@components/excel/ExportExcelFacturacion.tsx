...
export default function ExportExcelFacturacion() {
  ...
  const {
    ...
    onSubmit: async (values) => {
      const response = await fetch(`${config.apiUrl}/facturacion/exportar/tabla-excel`, {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    if (!usuario) return
    setFieldValue('cliente', `${usuario?.nombres} ${usuario?.apellido_paterno}`)
    ...
    <div>
      <form onSubmit={handleSubmit}>
        <h2 className="mb-8 text-3xl font-medium text-center font_kanit text-secondary-main">
          Filtros de Exportación - Facturación
        </h2>
